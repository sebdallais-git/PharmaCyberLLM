...
## Amendments from source verification (2026-09-17)

Hermes Agent source was read at commit `228022ef5b209cb0a3d739394edddf887e1db0f6` (version 0.21.3) before planning. Nothing was installed. These findings settle the open points above and override conflicting details:

| Topic | Verified fact | Effect on this design |
|---|---|---|
| SSRF | `security.allow_private_urls` is checked only by web extract, browser, vision, media and skills-hub fetches. Model base URLs, MCP URLs and `SEARXNG_URL` are not checked. | Keep `allow_private_urls: false`. localhost model and MCP URLs work, and web extract cannot reach ChromaDB or Neo4j. |
| Browser | The local browser backend skips the private-URL check. | Browser toolset disabled. |
| Web search | `web.search_backend: searxng` with `SEARXNG_URL`, no key. `web.keyless_fallback` and `web.keyless_rescue` default to true and retry on cloud free tiers. | Use the local SearXNG (`http://localhost:8888`). Set both fallbacks to false. |
| Custom provider | `providers.<name>` with `api`, `key_env`, `discover_models`, `request_timeout_seconds`, `stale_timeout_seconds`, `models.<id>.context_length`; selected with `model.provider: custom:<name>` and `model.context_length`. Auxiliary tasks on `auto` use the main provider. Minimum context is 64,000. | Timeouts are set per provider (`request_timeout_seconds: 1800`, `stale_timeout_seconds: 900`) instead of `HERMES_STREAM_READ_TIMEOUT`. |
| MCP | Keys `url`, `headers` (with `${VAR}`), `connect_timeout`, `timeout` (per call, default 300), `tools.include/exclude/resources/prompts`. Tools are named `mcp__pharmallm__<tool>`. **The Streamable HTTP client uses an httpx read timeout of 300 s between bytes** (`tools/mcp_tool_transport.py:434`). | **New requirement:** `pharmallm-mcp` must send an MCP `notifications/message` keepalive on the call's response stream at most every 60 s while `run_news_agent`, `ask_pharmallm` or `resolve_knowledge_gap` runs, and declare the `logging` capability. Otherwise calls longer than 300 s fail. |
| Toolsets | `platform_toolsets.<cli\|telegram\|cron>` lists. There is no `mcp` toolset: naming an MCP server in the list allowlists it. `agent.disabled_toolsets` removes toolsets globally. File tools and `execute_code` run through the terminal backend; web, memory and skills run on the host. | Telegram gets no `cronjob` toolset (injected content cannot create jobs). Cron runs get no terminal or file tools. `browser`, `computer_use`, `code_execution`, `image_gen`, `tts`, `delegation` disabled. |
| Docker sandbox | `terminal.docker_network: false` adds `--network=none`. `docker_mount_cwd_to_workspace` defaults to false. Hermes always mounts its credential, skill and cache dirs read-only into the container. | Network none is supported. Live verification checks what `/root/.hermes` exposes inside the container and records it. |
| Approvals | `approvals.mode` (`smart` default), `cron_mode`, `single_query_mode`, `unattended_mode` (all default `deny`); `skills.write_approval`, `memory.write_approval`. | As designed; set explicitly. |
| Telegram | `TELEGRAM_BOT_TOKEN`, `TELEGRAM_ALLOWED_USERS`, `TELEGRAM_HOME_CHANNEL`. With an allowlist set, unknown users are ignored (no pairing). | `TELEGRAM_HOME_CHANNEL` is set to the user's ID so `--deliver telegram` reaches the DM. `unauthorized_dm_behavior: ignore` is set explicitly. |
| Cron | `hermes cron create SCHEDULE PROMPT --name N --deliver telegram`; `hermes cron edit JOB_ID --schedule --prompt --deliver` (no update command); jobs stored in `$HERMES_HOME/cron/jobs.json` as `{"jobs": [{"id", "name", …}]}`. A reply of `[SILENT]` suppresses delivery; failures still deliver. The scheduler runs inside the gateway. | Health watch replies `[SILENT]` when healthy (the fallback is not needed). The job definitions live in `hermes/cron/jobs.json` (JSON, readable with the Python standard library), not YAML. The setup script reads Hermes' `jobs.json` read-only to find job IDs by name. |
| Installer | Installs uv and Python 3.11–3.13, clones the repo, runs `uv sync`, installs Playwright Chromium (skip: `--skip-browser`), pipes a third-party `cua-driver` installer into bash on macOS (skip: `--skip-computer-use`), appends PATH lines to `~/.zshrc`, `~/.zprofile` and `~/.profile`, runs a setup wizard (skip: `--skip-setup`), supports `--commit SHA`. | Install from the pinned commit with `--skip-setup --skip-browser --skip-computer-use`, after showing the user the script. The PATH edits are expected. |
| launchd | `hermes gateway install --force --start-now --start-on-login` writes `~/Library/LaunchAgents/ai.hermes.gateway.plist`; secrets are read from `.env`; logs go to `~/.hermes/logs/gateway.log`. | As designed. |
| Node for launchd | Node comes from nvm and is not on launchd's PATH. | The `com.pharmallm.mcp` plist carries `NODE_BIN` (absolute path, not a secret), and `run-mcp.sh` uses it. |
